...
1. Compile resources if any of the resources such as svg files or ui files are modified or added.   
   `glib-compile-resources --target=resources.cc --generate-source resources.xml`
2. `make` - Build a debug build..
3. `make BUILD=release` - Build a release build.
4. `make BUILD=sanitize_address` - Build with address sanitizer.
5. `make CPPFLAGS="-DLOCAL_DEV"` - Build for local development using web server running on localhost.
